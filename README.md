...
| 지점별 예보 | 지역명, 현재 위치 또는 위·경도로 원하는 지점의 풍향·풍속과 시간대별 상세예보 확인 |
...
| 지도 도구 | 거리 측정, 대표 지점 표시, 배경지도 선택, 현재 화면 공유, 밝은·어두운 테마 전환 |
...
4. 지역을 검색하거나 현재 위치 버튼을 눌러 지점별 48시간 예보를 엽니다.
5. 공유 버튼으로 현재 지도 중심과 확대 수준을 포함한 링크를 복사합니다.
6. 더 자세히 보고 싶다면 `설정`에서 표현 방식을 바꾸거나 `3D` 지도를 사용합니다.
...
| `VWORLD_API_KEY` | Spring 배포의 VWorld 배경지도 및 벡터 도로 | 선택 |

VWorld 키에는 실제 실행 도메인을 등록하고 필요한 2D 지도·배경지도·WMTS/TMS 권한을
허용해야 합니다. Cloudflare 배포는 VWorld 원점의 edge 요청 제약 때문에 OpenStreetMap을
기본 배경지도로 사용합니다.
...
이 저장소에는 별도 오픈소스 라이선스가 부여되지 않았으므로 재사용 전 저장소 소유자의
허가가 필요합니다.
