...
아래 이미지는 저장소의 애플리케이션을 1440×900 브라우저에서 직접 실행해 캡처했습니다.
바람·기온·강수·3D 화면은 2026년 7월 23일 실데이터 모드에서 확인한 화면입니다.
...
지역 검색으로 연 실제 화면입니다. 이 캡처의 풍향·풍속 시계열은 로컬 데모 자료를
사용했으며, 하단 상세예보는 외부 기상청 응답이 없어 오류 안내가 표시된 상태입니다.
...
| 지점 예보 | 48시간 풍향·풍속 차트, 시간대별 상세예보 | 차트 동작 확인, 상세예보는 기상청 API 연결 필요 |
| 대기질 | PM10, PM2.5 측정소 | AirKorea 자격 증명이 있는 환경에서 사용 |
| 교통 | ITS 도로 CCTV와 HLS 영상 | ITS 자격 증명이 있는 환경에서 사용 |
| 위험기상 | 기상특보, 태풍 경로, 최근 낙뢰 | 태풍·낙뢰 연결 작업 중 |
| 도로 배경지도 | Kakao Maps, OpenStreetMap 대체지도 | Kakao 사용 불가 시 OpenStreetMap으로 자동 전환 |
