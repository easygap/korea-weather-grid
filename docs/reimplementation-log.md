...
## 2026-08-05 · CCTV PoP 냉시작 복구와 KV 예산 보호

- 원인 확인: 동일한 인증키와 `:9443/cctvInfo` 요청이 로컬에서는 200으로 응답하지만 운영
  Worker의 서로 다른 Cloudflare 접속 지점에서는 반복해서 503이 발생했다. 기존 Cache API
  상태는 PoP별이라 한 지점의 정상 스냅샷이 다른 지점의 냉시작 장애를 복구하지 못했다.
- 설계: 같은 PoP의 정상 위치는 1분 fresh·30분 stale로 유지하고, 비어 있지 않은 첫 성공
  supertile만 기존 KV에 하루 한 번 이하로 저장한다. 새 PoP에서 실시간 ITS가 실패하면 24시간
  이내 공유 위치를 검증해 사용하고, 공유 자료는 항상 `stale:true`로 반환해 오래된 HLS가
  자동 재생되지 않게 했다. 전역 차단기는 공개 `Retry-After`와 같은 1분으로 맞췄다. 첫 배포의
  냉시작도 복구할 수 있도록 운영 초기화 스크립트가 Worker의 정규화·캐시 검증 함수를 그대로
  사용해 공식 ITS 원문을 0.5° 타일로 분할하고 인증키 없는 값만 KV에 기록한다.
- 예산·보안: 빈 타일은 KV에 쓰지 않고 요청당 공유 write를 최대 1회로 제한했다. 14개
  supertile 최악 경로는 성공 시 Cache API·상류·KV 합계 45회, 전부 실패 시 44회로 50회
  한도 안에 둔다. KV 값도 ITS 정규화와 타일 좌표 검증을 다시 통과해야 하며 인증키는 캐시
  키·값·로그 어디에도 저장하지 않는다.
- 검증: 냉시작 공유 복구, 24시간 초과 거부, 빈 타일 미저장, 일일 갱신 제한, 오래된 영상
  재생 차단과 부분 장애를 합성 계약으로 고정했다. Spring 172개, Worker·정적 셸 242개,
  실제 Chromium E2E 81개를 전체 품질 게이트에서 다시 확인했다. 운영 RC에서는 공식 원문
  4,739건을 누락 없이 42개 타일로 검증해 초기화했고, 부산 표본이 `stale:true`,
  `partial:false` 200 응답으로 복구되는 것을 확인했다.
...
## 2026-08-06 · 초기 로딩과 테마 전환 성능 정리

- 원인 확인: 운영 모바일 제한 trace에서 첫 화면 LCP가 2.925초였고, 기상특보 요약이
  `requestIdleCallback`의 2.5초 timeout 뒤에 나타나 전체 렌더 지연의 대부분을 차지했다.
  테마 전환은 화면 크기가 바뀌지 않아도 OpenLayers `updateSize()`를 호출해 4배 CPU 제한
  조건에서 39ms의 강제 레이아웃을 만들었다.
- 의존성 정리: 초기화와 발표시각 컨트롤에 남아 있던 jQuery 사용을 네이티브 DOM 이벤트와
  명시적 `DOMContentLoaded` 처리로 교체했다. 배포하지 않는 jQuery 본문·라이선스·고지와
  vendor lock 항목을 함께 제거하고, 두 셸에서 런타임 참조가 다시 생기지 않도록 계약 테스트를
  추가했다.
- 렌더링 경로: 특보는 첫 화면 안전 정보이므로 비동기 요청을 즉시 시작한다. CSS 토큰은
  `@import` 대신 독립 stylesheet로 연결하고 첫 화면에 쓰는 IBM Plex Mono 600을 미리
  요청한다. 풍장에는 크기 재측정이 필요한 `refresh`와 현재 크기로 다시 그리는 `repaint`를
  분리해 테마 전환이 레이아웃 측정을 유발하지 않게 했다.
- 검증: 같은 모바일·Slow 4G·4배 CPU 실험실 조건의 최적화 셸은 LCP 1.598초를 기록했다.
  서로 다른 원점에서 측정한 운영 기준선과 절대 비교하지 않고 회귀 판단용으로만 사용한다.
  테마 전환 INP는 128ms에서 93ms로 낮아졌고 CLS는 0.00, 강제 레이아웃 항목은 미검출이었다.
  Spring 172개, Worker·정적 셸 244개, 실제 Chromium E2E 81개와 세 npm 작업공간의
  취약점 감사를 통과했으며 Worker dry-run은 95개 자산과 gzip 35.37 KiB 번들을 확인했다.
