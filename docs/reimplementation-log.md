...
- 범위: Worker·Spring의 VWorld 런타임 설정과 타일 경로, 업스트림 URL 검증, 브라우저 CSP,
  Jackson·Netty 관리 버전.
- 설계: `/api/runtime/map-config`는 키 대신 고정된 `/api/map/vworld` 경로만 반환한다.
  타일 프록시는 `base` PNG와 `traffic` PBF의 정해진 경로·확대 단계·국내 인접 범위만
  허용하고, 응답 종류와 실제 바이트 상한을 확인한 뒤 키 없는 내부 캐시 키로 저장한다.
  자격 증명 URL이 포함될 수 있는 상류 예외 원인은 보존하거나 로그에 남기지 않는다.
...
- 검증: Worker·Spring 계약 테스트는 런타임 키 비노출, 허용 타일 중계, 잘못된 경로·쿼리·
  지역·응답 종류·크기 거부, 오류 내용 비노출과 HTTPS·CSP 정책을 검사한다.
