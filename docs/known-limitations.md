...
기준일은 2026-09-22이다. 이전 운영 점검 결과에는 해당 확인 날짜를 따로 적었다.
...
Spring 172개, Worker·정적 셸 244개, 실제 Chromium E2E 81개 계약 테스트를 통과한
2026-08-06 기준으로 완료했으며, 이후 변경은 같은 품질 게이트로 다시 검증한다.
...
2026년 9월 22일 화면 개편 검증에서는 운영 API의 바람·기온·일사강도, 지점별 48시간 예보,
AirKorea 관측 응답을 확인했다. 같은 날 서울 지역을 확대해 조회한 ITS CCTV는 연결 지연으로
목록을 받지 못했다. 새 화면은 확대 안내·조회 상태·실패 후 재시도를 지도 위에 표시한다.
이날의 CCTV 정상 연결을 주장하거나 이전 화면을 새 캡처로 사용하지 않는다.
