...
| `static/js/weather-grid-map-state.js`, `static/js/weather-grid-projection.js`, `static/js/weather-grid-readout.js` | complete | 세 도법 전이·줌 한계·남북 격자 표본을 순수 모델로 계산하고, OpenLayers 뷰·레이어 재투영과 60ms 포인터 리드아웃을 분리된 어댑터로 Node·브라우저 검증 |
...
| `static/js/weather-grid-layer-state.js`, `static/js/weather-grid-layers.js` | complete | 색상·흐름선·등치선 선택과 요소별 가용성을 불변 상태로 작성. 범주형 색상 필수, 비바람 흐름선 차단, 래스터·범례·등치선·입자 효과와 ARIA를 Node·브라우저로 검증 |
...
| Kakao 도로 배경지도 | implemented · RC verification pending | 공식 Map Web SDK 동적 로더, 환경변수 런타임 설정, OpenLayers 중심·확대 수준 동기화, CSP, 미설정·403 fallback과 순수 상태·서버·Worker 계약 테스트 완료. Kakao Developers의 Map API 사용 설정 ON 승인과 실제 도메인 SDK·타일 E2E만 대기 |
